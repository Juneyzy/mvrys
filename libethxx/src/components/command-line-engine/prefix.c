...
prefix_copy(struct prefix *dest, const struct prefix *src)
{
    ...
    {
        fprintf(stdout, "Error! prefix_copy(): Unknown address family %d",
                src->family);
        assert(0);
    }
}
...
str2prefix_ipv4(const char *str, struct prefix_ipv4 *p)
{
    ...
    if (pnt == NULL)
    {
        ...
        ret = cli_inet_aton(str, &p->prefix);
                       ...
                       char *prefix_str)
{
    ...
    u_int32_t destination;
    int ret;
    ret = cli_inet_aton(net_str, &network);
    ...
    if (mask_str)
    {
        ret = cli_inet_aton(mask_str, &mask);
