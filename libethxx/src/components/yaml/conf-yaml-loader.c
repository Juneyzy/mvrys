...
#define yaml_error(fmt,args...) printf(fmt,##args)
#define yaml_debug(fmt,args...) //printf(fmt,##args)
...
ConfYamlHandleInclude(ConfNode *parent, const char *filename)
{
    ...
    if (yaml_parser_initialize(&parser) != 1) {
        yaml_error("Failed to initialize YAML parser");
        return -1;
    }
...
ConfYamlParse(yaml_parser_t *parser, ConfNode *parent, int inseq)
{
    ...
    while (!done) {
        ...
        if (event.type == YAML_DOCUMENT_START_EVENT) {
            yaml_debug("event.type=YAML_DOCUMENT_START_EVENT; state=%d\n", state);
            ...
            yaml_debug("event.type=YAML_SCALAR_EVENT; state=%d; value=%s; "
            ...
            if (inseq) {
                ...
                if (state == CONF_INCLUDE) {
                    yaml_debug("Including configuration file %s.\n", value);
                    ...
                    if (existing != NULL) {
                        if (!existing->final) {
                            yaml_error("Configuration node '%s' redefined.\n",
                                existing->name);
                    ...
                    if ((tag != NULL) && (strcmp(tag, "!include") == 0)) {
                        yaml_error("Including configuration file %s at "
            ...
            yaml_debug("event.type=YAML_SEQUENCE_START_EVENT; state=%d\n", state);
            if (ConfYamlParse(parser, node, 1) != 0)
                goto fail;
            state = CONF_KEY;
        }
        else if (event.type == YAML_SEQUENCE_END_EVENT) {
            yaml_debug("event.type=YAML_SEQUENCE_END_EVENT; state=%d\n", state);
            return 0;
        }
        else if (event.type == YAML_MAPPING_START_EVENT) {
            yaml_debug("event.type=YAML_MAPPING_START_EVENT; state=%d\n", state);
            if (inseq) {
                char sequence_node_name[DEFAULT_NAME_LEN];
                snprintf(sequence_node_name, DEFAULT_NAME_LEN, "%d", seq_idx++);
            ...
            yaml_debug("event.type=YAML_MAPPING_END_EVENT; state=%d\n", state);
            done = 1;
        }
        else if (event.type == YAML_STREAM_END_EVENT) {
            yaml_debug("event.type=YAML_STREAM_END_EVENT; state=%d\n", state);
            done = 1;
        }
...
ConfYamlLoadFile(const char *filename)
{
    ...
    if (stat(filename, &stat_buf) == 0) {
        if (stat_buf.st_mode & S_IFDIR) {
            yaml_error("yaml argument is not a file but a directory: %s. "
    ...
    if (infile == NULL) {
        yaml_error("failed to open file: %s: %s\n", filename,
            strerror(errno));
        yaml_parser_delete(&parser);
        return -1;
    }
