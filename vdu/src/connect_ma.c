...
int reload_vrs_conf(char *filename)
{
			...
			TAILQ_FOREACH(subchild, &child->head, next) {
				...
				if (0 == strcmp(subchild->name, "id")) {
					...
					if (-1 == check_ip_format(subchild->val)) {
						applog(APP_LOG_LEVEL_ERR, VDU_LOG_MASK_BASE, "the ip format is error");
						exit(1);
					}
					if (tmpconf.ip == NULL)
	...
	if (tmpconf.vrsnum != 0) {
		...
		//bzero(tmpvrs, 0);
        memset(tmpvrs, 0, tmpconf.vrsnum * sizeof(vrs_t));
			...
			TAILQ_FOREACH(subchild, &child->head, next) {
				...
				if (!strcmp(subchild->name, "mac")) {
					if (-1 == check_mac_format(subchild->val)) {
						applog(APP_LOG_LEVEL_ERR, VDU_LOG_MASK_BASE, "the mac format is error");
						exit(1);
					}
	...
	for (i = 0; i < tmpconf->vrsnum; i ++) {
		for (j = 0; j < conf_para.vrsnum; j ++) {
			if (vrs_msg[j].vrsid == tmpvrs[i].vrsid && vrs_msg[j].state == RUNNING) {
				...
				if (i != j) {
					change_call_msg_index(i, vrs_msg[j].vrsid, vrs_msg[i].vrsid);
				}
			}
		}
	}
