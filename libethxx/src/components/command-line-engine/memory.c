...
{
    fprintf(stderr, "Eroor! %s : can't allocate memory for `%s' size %d: %s\n",
...
void *
pas_zcalloc(int type, size_t size)
{
    void *memory;
...
log_memstats(int pri)
{
    ...
    for (ml = mlists; ml->list; ml++)
    {
        ...
        fprintf(stdout, "Memory utilization in module %s:", ml->name);

        for (m = ml->list; m->index >= 0; m++)
            if (m->index && mstat[m->index].alloc)
                fprintf(stdout, "  %-22s: %5ld", m->format, mstat[m->index].alloc);
    }
}
