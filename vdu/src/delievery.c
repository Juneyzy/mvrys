...
void vdu_print_statistics_info()
{
    applog(APP_LOG_LEVEL_INFO, VDU_LOG_MASK_BASE, "---------------------------------\n");
    applog(APP_LOG_LEVEL_INFO, VDU_LOG_MASK_BASE, "all_packet           :    %-lu\n", atomic64_read(&all_packet));
    applog(APP_LOG_LEVEL_INFO, VDU_LOG_MASK_BASE, "unhandle_packet      :    %-lu\n", atomic64_read(&unhandle_packet));
    applog(APP_LOG_LEVEL_INFO, VDU_LOG_MASK_BASE, "pcap_packet          :    %-lu\n", atomic64_read(&pcap_packet));
    applog(APP_LOG_LEVEL_INFO, VDU_LOG_MASK_BASE, "recv_packet          :    %-lu\n", atomic64_read(&recv_packet));
    applog(APP_LOG_LEVEL_INFO, VDU_LOG_MASK_BASE, "send_packet          :    %-lu\n", atomic64_read(&send_packet));
    applog(APP_LOG_LEVEL_INFO, VDU_LOG_MASK_BASE, "handle_packet        :    %-lu\n", atomic64_read(&handle_packet));
    applog(APP_LOG_LEVEL_INFO, VDU_LOG_MASK_BASE, "lose_packet          :    %-lu\n", atomic64_read(&lose_packet));
    applog(APP_LOG_LEVEL_INFO, VDU_LOG_MASK_BASE, "sessnum              :    %-lu\n", atomic64_read(&sessnum));
    applog(APP_LOG_LEVEL_INFO, VDU_LOG_MASK_BASE, "sessover             :    %-lu\n", atomic64_read(&sessover));
    applog(APP_LOG_LEVEL_INFO, VDU_LOG_MASK_BASE, "sessovertime         :    %-lu\n", atomic64_read(&sessovertime));
    applog(APP_LOG_LEVEL_INFO, VDU_LOG_MASK_BASE, "cmdstop              :    %-lu\n", atomic64_read(&cmdstop));
    applog(APP_LOG_LEVEL_INFO, VDU_LOG_MASK_BASE, "confupdate           :    %-lu\n", atomic64_read(&confupdate));
    applog(APP_LOG_LEVEL_INFO, VDU_LOG_MASK_BASE, "cmdstatus            :    %-lu\n", atomic64_read(&cmdstatus));
    applog(APP_LOG_LEVEL_INFO, VDU_LOG_MASK_BASE, "cmdcounter           :    %-lu\n", atomic64_read(&cmdcounter));
}

void vdu_clear_statistics_info()
{
    init_atomic_counter();
...
void *thread_delivery(void *arg)
{
	...
	dev_vrs = pcap_open_live(conf_para.vrs_nic, PACKET_LENGTH, PROMISCUOUS_STATE, OVERTIME, errbuf);
	if (dev_vrs == NULL) {
...
//void delievery_send_to_vrs(void *arg)
{
	...
	if (call_info[sr155_no][e1_no][ts_no].callid == 0) {
        ...
        memset(call_info[sr155_no][e1_no][ts_no].data_len, 0, VOICE_NUM * STOR_TIME * sizeof(uint16_t));
        ...
        memset(call_info[sr155_no][e1_no][ts_no].voice_data, 0, VOICE_NUM * STOR_TIME * sizeof(data_t));
		...
		for (i = 0; i < conf_para.vrsnum; i ++) {
			...
			if (session_num == 0 || session_num > vrs_msg[i].session_num) {
				session_num = vrs_msg[i].session_num;    /** 选择会话数目最少的vrs */
				vrsid = vrs_msg[i].vrsid;
				if (vrs_msg[i].session_num == 0)
					break;
			}
		}
		if (vrsid == -1) {
		...
		applog(APP_LOG_LEVEL_DEBUG, VDU_LOG_MASK_BASE, "[NEW Session] callid=%lu, vrsid=%d", callid, vrsid);
		...
		for (i = 0; i < conf_para.vrsnum; i ++) {
			if (vrsid == vrs_msg[i].vrsid) {
				if (vrs_msg[i].session_num >= SR155_NUM * E1_NUM * TS_NUM)
					return 0;
		...
		for (i = 0; i < vrs_msg[call_info[sr155_no][e1_no][ts_no].vrsindex].session_num; i ++) {
			if (vrs_msg[call_info[sr155_no][e1_no][ts_no].vrsindex].callid[i] == call_info[sr155_no][e1_no][ts_no].callid) {
				for (j = i; j < vrs_msg[call_info[sr155_no][e1_no][ts_no].vrsindex].session_num - 1; j ++) {
					if (vrs_msg[call_info[sr155_no][e1_no][ts_no].vrsindex].session_num >= SR155_NUM * E1_NUM * TS_NUM)
						return 0;
		...
		applog(APP_LOG_LEVEL_DEBUG, VDU_LOG_MASK_BASE, "[COVRE Session] callid=%lu, vrsid=%d", callid, vrsid);
		call_info[sr155_no][e1_no][ts_no].vrsid = vrsid;
		call_info[sr155_no][e1_no][ts_no].starttime = time(NULL);
		call_info[sr155_no][e1_no][ts_no].count ++;
		for (i = 0; i < conf_para.vrsnum; i ++) {
	...
	for (l = 0; l < conf_para.vrsnum; l ++)
		syslog(LOG_INFO, "l = %d vrsid = %d stauts = %d sessionnum = %lu mac : %02x:%02x:%02x:%02x:%02x:%02x", call_info[sr155_no][e1_no][ts_no].vrsindex,
				vrs_msg[call_info[sr155_no][e1_no][ts_no].vrsindex].vrsid, vrs_msg[call_info[sr155_no][e1_no][ts_no].vrsindex].state,
				vrs_msg[call_info[sr155_no][e1_no][ts_no].vrsindex].session_num, vrs_msg[call_info[sr155_no][e1_no][ts_no].vrsindex].vrs_mac[0],
				vrs_msg[call_info[sr155_no][e1_no][ts_no].vrsindex].vrs_mac[1], vrs_msg[call_info[sr155_no][e1_no][ts_no].vrsindex].vrs_mac[2],
				vrs_msg[call_info[sr155_no][e1_no][ts_no].vrsindex].vrs_mac[3], vrs_msg[call_info[sr155_no][e1_no][ts_no].vrsindex].vrs_mac[4],
				vrs_msg[call_info[sr155_no][e1_no][ts_no].vrsindex].vrs_mac[5]);
#endif
...
STOP_FLAG:
	...
	if (call_info[sr155_no][e1_no][ts_no].endtimes == 2) {
		...
		applog(APP_LOG_LEVEL_DEBUG, VDU_LOG_MASK_BASE, "[STOP Session] callid=%lu, size=%u", callid, call_info[sr155_no][e1_no][ts_no].count);
		vrs_msg[call_info[sr155_no][e1_no][ts_no].vrsindex].session_num --;
		//	init_call_msg(&call_info[sr155_no][e1_no][ts_no]);
...
int init_call_msg(session_t *data)
{
    ...
    memset(data->data_len, 0, VOICE_NUM * STOR_TIME * sizeof(uint16_t));
	data->starttime = 0;
	//bzero(data->voice_data, 0);
    memset(data->voice_data, 0, VOICE_NUM * STOR_TIME * sizeof(data_t));
	return 0;
}
...
int get_vrs_conf(void)
{
	...
	if (conf_para.vrsnum != 0) {
		...
		//bzero(vrs_msg, 0);
        memset(vrs_msg, 0, conf_para.vrsnum * sizeof(vrs_t));
		...
		TAILQ_FOREACH(child, &base->head, next) {
			TAILQ_FOREACH(subchild, &child->head, next) {
				...
				if (!strcmp(subchild->name, "mac")) {
					...
					printf("i = %d %s : %d\n", i, subchild->name, vrs_msg[i].vrsid);
			...
			if (j != conf_para.vrsnum) {
				//vrs_msg[i].state = RUNNING;    /** vpw的状态由ma确定下发 */
				i ++;
			}
		}
	}
	...
	for (i = 0; i < conf_para.vrsnum; i ++)
		printf("vrsid = %d state = %d\n", vrs_msg[i].vrsid, vrs_msg[i].state);
...
//		conf_para.maip = (char *)malloc(strlen(SERV_IP) + 1);
	//strncpy(conf_para.maip, SERV_IP, strlen(SERV_IP) + 1);
	strcpy(conf_para.maip, SERV_IP);
...
//		conf_para.sguardip = (char *)malloc(strlen(SERV_IP) + 1);
	//strncpy(conf_para.sguardip, SERV_IP, strlen(SERV_IP) + 1);
	strcpy(conf_para.sguardip, SERV_IP);
		...
		if (p->vrsid == id) {
			if (p->state == RUNNING && status != RUNNING) {
				...
				if (vrsid == -1) {
					...
					//bzero(p->callid, 0);
                    memset(p->callid, 0, sizeof(p->callid));
					p->session_num = 0;
					return -1;
				}
				change_call_msg_index(i, id, vrsid);
...
int send_voice_data_to_another_vrs(int srcid, int dstid)
{
	...
	for (i = 0; i < vrs_msg[src].session_num; i ++) {
		...
		call_info[sr155_no][e1_no][ts_no].vrsindex = dst;
		for (j = 0; j < call_info[sr155_no][e1_no][ts_no].count; j ++) {
			length = 0;
			memcpy(buff, vrs_msg[call_info[sr155_no][e1_no][ts_no].vrsindex].vrs_mac, 6);
			memcpy(buff + 6, conf_para.tovrs_mac, 6);
			length += 12;
			if (call_info[sr155_no][e1_no][ts_no].voice_data[j].vdata != NULL) {
				length += call_info[sr155_no][e1_no][ts_no].data_len[j];
				memcpy(buff + 12, call_info[sr155_no][e1_no][ts_no].voice_data[j].vdata, call_info[sr155_no][e1_no][ts_no].data_len[j]);
			}
			pcap_inject(dev_vrs, buff, length);
		}
    ...
    memset(vrs_msg[src].callid, 0, sizeof(vrs_msg[src].callid));
...
int stop_session_by_callid(uint64_t callid)
{
	...
	if (call_info[sr155_no][e1_no][ts_no].callid == callid) {
		...
		applog(APP_LOG_LEVEL_INFO, VDU_LOG_MASK_BASE, "get a stop callid = %lu session", callid);
...
int change_call_msg_index(int index, int vrsid, int dstid)
{
	int i, j, k;
	for (i = 0; i < SR155_NUM; i ++) {
		for (j = 0; j < E1_NUM; j ++) {
